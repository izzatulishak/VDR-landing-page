import{aa as o,af as pe,a9 as T,ks as ue,c6 as Z,pb as fe,pa as ge,fV as _e,cI as J,Bv as me,Bs as ve,B_ as Pe,zD as f,AM as w,Bx as be,zE as j,zB as r,cm as U,zt as Q,Bu as we,Au as C,p4 as Ve,cJ as m,dz as $,ee as B,B$ as De,kd as Y,AK as ee,Aa as S,p8 as xe,cK as v,C0 as te,iK as ie,dc as ne,ed as Ee,eb as ye,cl as Le,ex as Ce,dy as Se,ab as se,p5 as Me,zF as ae,zG as re,c as le,BB as Ae,zs as $e,cH as N,ep as O,Bt as Re,rU as Te,f0 as ze,A8 as Oe,AU as Ue,zA as W,C1 as Ie,_ as u,zU as D,BC as qe,BD as H,C2 as je,oH as Be,C3 as R,C4 as Ne,er as We,es as He,qR as Fe,eu as Ge,ev as ke,ew as Ke,m as M,b as Xe,C5 as Ze,eZ as F}from"./index-CeBxY8tu.js";import{t as Je}from"./VisualElement-CCBVpOGm.js";let G=class{constructor(e){this._renderCoordsHelper=e,this._origin=o(),this._dirty=!1,this._count=0}set vertices(e){const n=pe(3*e.length);let i=0;for(const s of e)n[i++]=s[0],n[i++]=s[1],n[i++]=s[2];this.buffers=[n]}set buffers(e){if(this._buffers=e,this._buffers.length>0){const n=this._buffers[0],i=3*Math.floor(n.length/3/2);T(this._origin,n[i],n[i+1],n[i+2])}else T(this._origin,0,0,0);this._dirty=!0}get origin(){return this._origin}draw(e){const n=this._ensureVAO(e);n!=null&&(e.bindVAO(n),e.drawArrays(ue.TRIANGLES,0,this._count))}dispose(){this._vao!=null&&this._vao.dispose()}get _layout(){return this._renderCoordsHelper.viewingMode===1?Ye:et}_ensureVAO(e){return this._buffers==null?null:(this._vao??(this._vao=this._createVAO(e,this._buffers)),this._ensureVertexData(this._vao,this._buffers),this._vao)}_createVAO(e,n){const i=this._createDataBuffer(n);return this._dirty=!1,new fe(e,new ge(e,_e(this._layout),i))}_ensureVertexData(e,n){var s;if(!this._dirty)return;const i=this._createDataBuffer(n);(s=e.buffer())==null||s.setData(i),this._dirty=!1}_createDataBuffer(e){const n=e.reduce((g,h)=>g+k(h),0);this._count=n;const i=this._layout.createBuffer(n),s=this._origin;let a=0,l=0;const c="startUp"in i?this._setUpVectors.bind(this,i):void 0;for(const g of e){for(let h=0;h<g.length;h+=3){const L=T(K,g[h],g[h+1],g[h+2]);h===0?l=this._renderCoordsHelper.getAltitude(L):this._renderCoordsHelper.setAltitude(L,l);const _=a+2*h;c==null||c(h,_,g,L);const q=J(K,L,s);if(h<g.length-3){for(let P=0;P<6;P++)i.start.setVec(_+P,q);i.extrude.setValues(_,0,-1),i.extrude.setValues(_+1,1,-1),i.extrude.setValues(_+2,1,1),i.extrude.setValues(_+3,0,-1),i.extrude.setValues(_+4,1,1),i.extrude.setValues(_+5,0,1)}if(h>0)for(let P=-6;P<0;P++)i.end.setVec(_+P,q)}a+=k(g)}return i.buffer}_setUpVectors(e,n,i,s,a){const l=this._renderCoordsHelper.worldUpAtPosition(a,Qe);if(n<s.length-3)for(let c=0;c<6;c++)e.startUp.setVec(i+c,l);if(n>0)for(let c=-6;c<0;c++)e.endUp.setVec(i+c,l)}};function k(t){return 3*(2*(t.length/3-1))}const Qe=o(),K=o(),Ye=Z().vec3f("start").vec3f("end").vec2f("extrude").vec3f("startUp").vec3f("endUp"),et=Z().vec3f("start").vec3f("end").vec2f("extrude");function oe(t,e){const n=t.fragment;n.include(me),t.include(ve),n.include(Pe),n.uniforms.add(new f("globalAlpha",i=>i.globalAlpha),new w("glowColor",i=>i.glowColor),new f("glowWidth",(i,s)=>i.glowWidth*s.camera.pixelRatio),new f("glowFalloff",i=>i.glowFalloff),new w("innerColor",i=>i.innerColor),new f("innerWidth",(i,s)=>i.innerWidth*s.camera.pixelRatio),new be("depthMap",i=>{var s;return(s=i.depth)==null?void 0:s.attachment}),new j("normalMap",i=>i.normals)),n.code.add(r`vec4 premultipliedColor(vec3 rgb, float alpha) {
...
return abs(dist);
}`),t.spherical){const i=(a,l,c)=>m(a,l.heightManifoldTarget,c.camera.viewMatrix),s=(a,l)=>m(a,[0,0,0],l.camera.viewMatrix);n.uniforms.add(new C("heightManifoldOrigin",(a,l)=>(i(p,a,l),s(V,l),J(V,V,p),$(d,V),d[3]=B(V),d)),new De("globalOrigin",a=>s(p,a)),new f("cosSphericalAngleThreshold",(a,l)=>1-Math.max(2,Y(l.camera.eye,a.heightManifoldTarget)*l.camera.perRenderPixelRatio)/B(a.heightManifoldTarget))),n.code.add(r`float globeDistancePixels(float posInGlobalOriginLength) {
...
vec4 color = vec4(0.0);`),t.heightManifoldEnabled){n.uniforms.add(new S("angleCutoff",s=>A(s)),new C("heightPlane",(s,a)=>he(s.heightManifoldTarget,s.renderCoordsHelper.worldUpAtPosition(s.heightManifoldTarget,p),a.camera.viewMatrix)));const i=t.spherical?r`normalize(globalOrigin - pos)`:r`heightPlane.xyz`;n.main.add(r`
